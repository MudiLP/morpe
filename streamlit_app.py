        ...
        if len(selected_items) == 1:
            ...
            with stats_col:
                ...
                def calculate_volatility(df, item):
                    ...
                    price_changes = df[item].pct_change()
                    
                    # Используем стандартное отклонение процентных изменений
                    volatility = price_changes.std() * 100
                    
                    # Нормализуем значение волатильности
                    volatility = min(volatility * 2, 100)  # ограничиваем максимальное значение
                ...
                # За всё время
                total_vol = calculate_volatility(filtered_df, item)  # используем новый метод расчета

                def get_volatility_status(vol):
                    if vol is None:
                        return "Нет данных", "gray"
                    elif vol < 15:  # изменили пороговые значения
                        return "Низкая", "green"
                    elif vol < 40:  # изменили пороговые значения
                ...
                **Интерпретация волатильности:**
                - 🟢 **Низкая** (<15%): Стабильный рынок, низкий риск, подходит для долгосрочных инвестиций
                - 🟡 **Средняя** (15-40%): Умеренные колебания, сбалансированный риск, подходит для среднесрочной торговли
                - 🔴 **Высокая** (>40%): Сильные колебания, высокий риск/потенциал, подходит для краткосрочной торговли
                """)
